...
from reporting.reporting.report.financial_statements import (get_period_list, get_columns, get_data)
from reporting.reporting.report.draft_profit_and_loss_statement.draft_profit_and_loss_statement import get_net_profit_loss
...
def get_account_type_based_data(company, account_type, period_list, accumulated_values ,filters):
	...
	for period in period_list:
		...
		account_by_type = frappe.db.sql_list("""SELECT name FROM tabAccount WHERE account_type = %s""",(account_type))
		draft_gl_entries = get_draft_gl_entries(filters,period['from_date'],period['from_date'])
		draft_amount = 0
		
		filterd_draft_gl_entries = []
		for draft_gl in draft_gl_entries :
			if (draft_gl.account,) in account_by_type:
				filterd_draft_gl_entries.append(draft_gl)
		
		for gl in filterd_draft_gl_entries:
			draft_amount += (gl.credit - gl.debit)
		...
		if filters.get("show_draft") !=1 and filters.get("show_submitted") != 1 :
			...
			total += draft_amount
			data.setdefault(period["key"], draft_amount)
			...
			total += (draft_amount+amount)
			data.setdefault(period["key"], (draft_amount+amount))
